...
const studentsOption = computed(() => ({
  ...
  series: [
    ...
    { name: '本科生就业率', type: 'line', yAxisIndex: 1, data: schools.value.map(s => s.undergraduateEmploymentRate) },
    { name: '硕士生就业率', type: 'line', yAxisIndex: 1, data: schools.value.map(s => s.masterEmploymentRate) },
    { name: '博士生就业率', type: 'line', yAxisIndex: 1, data: schools.value.map(s => s.doctoralEmploymentRate) }
  ]
}));
